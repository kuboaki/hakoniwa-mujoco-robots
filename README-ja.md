...
2. シミュレーションを実行します（`raspi/sonar_radar.py` 相当のロジックを
   `libspikehat_sim` 上でそのまま実行し、JSON配列を標準出力に出力します）。

```bash
python3 python/spikehat/sonar_radar/sonar_radar_sim.py
```

3. ビューア付き（macOSは `mjpython` 推奨、等倍速固定）:

```bash
mjpython python/spikehat/sonar_radar/sonar_radar_sim.py --viewer
```
